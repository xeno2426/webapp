...
def create_user(username,password_hash,bio="",avatar="",recovery=""):
    execute("INSERT INTO users(username,password_hash,bio,avatar,recovery) VALUES(:1,:2,:3,:4,:5) ON CONFLICT(username) DO NOTHING",(username,password_hash,bio,avatar,recovery))
def update_user(username,**kwargs):
    allowed={"password_hash","bio","avatar","recovery","last_seen","typing_to","typing_ts","typing_group","typing_group_ts"}
    conn=get_conn()
    try:
        for k,v in kwargs.items():
            if k in allowed: conn.run(f"UPDATE users SET {k}=:1 WHERE username=:2",v,username)
    finally: conn.close()
def delete_user(username):
    for sql in ["DELETE FROM friend_requests WHERE from_user=:1 OR to_user=:1",
                "DELETE FROM friends WHERE user1=:1 OR user2=:1",
                "DELETE FROM unread WHERE username=:1 OR from_user=:1",
                "DELETE FROM notes WHERE username=:1","DELETE FROM stories WHERE username=:1",
                "DELETE FROM group_members WHERE username=:1","DELETE FROM users WHERE username=:1"]:
...
def are_friends(u1,u2): return bool(query("SELECT 1 FROM friends WHERE (user1=:1 AND user2=:2) OR (user1=:2 AND user2=:1)",(u1,u2),one=True))
def add_friend(u1,u2):
    a,b=sorted([u1,u2]); execute("INSERT INTO friends(user1,user2) VALUES(:1,:2) ON CONFLICT DO NOTHING",(a,b))
def remove_friend(u1,u2):
    a,b=sorted([u1,u2]); execute("DELETE FROM friends WHERE user1=:1 AND user2=:2",(a,b))
...
def already_requested(f,t): return bool(query("SELECT 1 FROM friend_requests WHERE from_user=:1 AND to_user=:2",(f,t),one=True))

# UNREAD
def count_unread(u):
    r=query("SELECT COALESCE(SUM(count),0) AS total FROM unread WHERE username=:1",(u,),one=True)
    return int(r["total"]) if r else 0
def get_unread_senders(u): return [r["from_user"] for r in query("SELECT from_user FROM unread WHERE username=:1 AND count>0",(u,))]
...
def mark_seen(sender,recipient): execute("UPDATE messages SET seen=TRUE WHERE sender=:1 AND recipient=:2 AND seen=FALSE",(sender,recipient))
def soft_delete_message(mid): execute("UPDATE messages SET deleted=TRUE,text='This message was deleted',ftype='text',filename='',url='',reactions='{}' WHERE id=:1",(mid,))
def get_message_by_id(mid):
    r=query("SELECT * FROM messages WHERE id=:1",(mid,),one=True)
...
def react_message(mid,username,emoji):
    ...
    execute("UPDATE messages SET reactions=:1 WHERE id=:2",(json.dumps(r),mid))
...
def soft_delete_group_message(mid): execute("UPDATE group_messages SET deleted=TRUE,text='This message was deleted',ftype='text',filename='',url='',reactions='{}' WHERE id=:1",(mid,))
def get_group_message_by_id(mid):
    r=query("SELECT * FROM group_messages WHERE id=:1",(mid,),one=True)
    if r and isinstance(r.get("reactions"),str): r["reactions"]=json.loads(r["reactions"])
    return r
def react_group_message(mid,username,emoji):
    ...
    execute("UPDATE group_messages SET reactions=:1 WHERE id=:2",(json.dumps(r),mid))

# NOTES
def load_notes(username,sort="newest"):
    order="DESC" if sort=="newest" else "ASC"
    return query(f"SELECT * FROM notes WHERE username=:1 ORDER BY created_at {order}",(username,))
def add_note(username,title,body): execute("INSERT INTO notes(username,title,body) VALUES(:1,:2,:3)",(username,title,body))
def update_note(note_id,username,title,body): execute("UPDATE notes SET title=:1,body=:2 WHERE id=:3 AND username=:4",(title,body,note_id,username))
def delete_note(note_id,username): execute("DELETE FROM notes WHERE id=:1 AND username=:2",(note_id,username))
...
def load_stories():
    ...
    return out
def add_story(username,url,ftype="image",caption=""): execute("INSERT INTO stories(username,url,ftype,caption,viewers) VALUES(:1,:2,:3,:4,:5)",(username,url,ftype,caption,"[]"))
def delete_story(story_id,username): execute("DELETE FROM stories WHERE id=:1 AND username=:2",(story_id,username))
def add_story_viewer(story_id,viewer):
    r=query("SELECT viewers FROM stories WHERE id=:1",(story_id,),one=True)
    if not r: return
    viewers=json.loads(r["viewers"]) if isinstance(r["viewers"],str) else (r["viewers"] or [])
    if viewer not in viewers:
        viewers.append(viewer)
        execute("UPDATE stories SET viewers=:1 WHERE id=:2",(json.dumps(viewers),story_id))
